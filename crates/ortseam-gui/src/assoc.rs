...
pub const PROG_ID: &str = "ortseam.spectrum";
...
pub fn registration(exe: &str) -> Vec<(String, String)> {
    let mut keys = vec![
        (PROG_ID.to_string(), "Gamma spectrum (ortseam)".to_string()),
...
/// are none of ortseam's business.
...
mod tests {
    ...
    fn every_spectrum_extension_points_at_the_prog_id() {
        let keys = registration(r"C:\tools\ortseam-gui.exe");
        for extension in EXTENSIONS {
            assert!(
                keys.iter()
    ...
    fn the_open_command_quotes_both_the_exe_and_the_file() {
        let keys = registration(r"C:\Program Files\ortseam\ortseam-gui.exe");
        let (_, command) = keys
            .iter()
            .find(|(key, _)| key.ends_with("shell\\open\\command"))
            .expect("an open command");
        assert_eq!(
            command, "\"C:\\Program Files\\ortseam\\ortseam-gui.exe\" \"%1\"",
