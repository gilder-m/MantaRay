...
fn scratch(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("ortseam-recall-tests");
    std::fs::create_dir_all(&dir).expect("scratch directory");
    dir.join(name)
}

/// A synthetic spectrum with a peak in it, written to a file ortseam can read.
fn written_spectrum(name: &str, channels: usize) -> PathBuf {
    ...
    ortseam_formats::save_spectrum(&spectrum, &path).expect("write the spectrum");
    path
}
...
fn every_writable_format_can_be_recalled_after_being_saved() {
    // The formats ortseam can write are the ones a user will hand back to it.
...
fn recalling_regions_and_a_calibration_applies_them_to_the_open_spectrum() {
    ...
    let mut regions = ortseam_core::RoiSet::default();
    regions.mark(ortseam_core::Roi::new(240, 270));
    let roi_path = scratch("regions.roi");
    ortseam_formats::save_rois(&regions, &roi_path).expect("write the region file");
...
fn the_fixture_spectra_all_open_through_the_application() {
    ...
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../ortseam-formats/tests/fixtures");
    ...
    let files: Vec<PathBuf> = entries
        ...
        .filter(|path| path.is_file() && ortseam_formats::SpectrumFormat::from_path(path).is_some())
        .collect();
    if files.is_empty() {
        eprintln!("no fixtures at {} - skipping", dir.display());
        return;
