...
//! way the person using ortseam would describe the step, so a failure names the
//! thing that broke rather than the function that broke.

use std::path::PathBuf;

use ortseam_core::{Roi, Spectrum};
use ortseam_device::Mcb;
use ortseam_gui::app::{Action, App, Target};
use ortseam_gui::view::MarkMode;

fn scratch(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("ortseam-session-tests");
...
fn a_spectrum_can_be_saved_and_the_window_stops_being_modified() {
    ...
    ortseam_formats::save_spectrum(&spectrum, &path).expect("save");
    app.windows[index].path = Some(path.clone());
    app.windows[index].modified = false;

    // What is on disk is what was on screen.
    let reread = ortseam_formats::load_spectrum(&path).expect("read it back");
...
fn stripping_a_background_leaves_less_than_was_there() {
    ...
    ortseam_formats::save_spectrum(&background, &path).expect("write the background");
...
fn an_uncalibrated_europium_spectrum_calibrates_itself_from_its_peaks() {
    ...
    let mut spectrum = ortseam_formats::load_spectrum(&fixture).expect("load");
...
fn a_job_can_walk_the_stored_spectra_with_loop_spectra_and_view() {
    ...
    ortseam_gui::jobs::start(&mut app, &job).expect("the job should parse");
    for _ in 0..100 {
...
fn a_network_instrument_joins_the_pick_list_and_counts() {
    ...
    let served: Arc<Mutex<ortseam_device::SimulatedMcb>> = Arc::new(Mutex::new(
        ortseam_device::SimulatedMcb::new(42, "FAR-SIDE"),
    ));
    ...
    app.add_detector(ortseam_device::DetectorEntry {
        number: 42,
        name: "BENCH".into(),
        model: "remote".into(),
    ...
    for _ in 0..4 {
        {
            ...
            ortseam_device::advance(&mut *instrument, 1.0).expect("far clock");
        }
        app.advance_by(1.0);
    }
    assert!(
...
fn removing_a_detector_closes_its_window_and_keeps_the_rest_straight() {
    ...
    assert!(
        app.windows.iter().any(
            |window| matches!(window.target, ortseam_gui::app::Target::Detector(index) if index == 0)
        ),
...
fn a_detector_kind_with_no_driver_behind_it_is_refused_not_simulated() {
    ...
    for (kind, expected) in [
        ...
        (ortseam_device::DetectorKind::FileReplay, "replay"),
    ] {
        let mut app = App::headless();
        let before = app.detectors.len();
        app.add_detector(ortseam_device::DetectorEntry {
...
fn the_look_and_the_recent_files_survive_a_restart() {
    ...
    ortseam_formats::save_spectrum(&Spectrum::new(64), &real).expect("write");
    let mut first = App::headless();
    first.theme = ortseam_gui::theme::Theme::AmberCrt;
    ...
    let persisted: ortseam_gui::app::Persisted = serde_json::from_str(&saved).expect("deserialise");
    second.restore(persisted);

    assert_eq!(second.theme, ortseam_gui::theme::Theme::AmberCrt);
...
fn decay_corrections_change_the_reported_activity_when_asked() {
    ...
    spectrum.energy_calibration = Some(ortseam_core::EnergyCalibration::linear(0.0, 1.0));
    ...
    app.library = ortseam_core::NuclideLibrary::new("test");
    app.library.nuclides.push(ortseam_core::Nuclide::new(
        "Xx-1",
        7200.0,
        vec![ortseam_core::LibraryPeak::new(140.0, 90.0)],
    ));
    app.efficiency = Some(ortseam_core::EfficiencyCalibration::constant(0.1));
...
fn the_last_spectrum_reopens_at_start_when_asked_to() {
    ...
    ortseam_formats::save_spectrum(&Spectrum::new(128), &path).expect("write");
...
fn a_scan_that_could_not_run_says_so_rather_than_reporting_nothing() {
    ...
    let scan = InstrumentScan::failed("ortseam-mcb.exe is not beside ortseam");
    assert!(scan.reachable.is_empty() && scan.missing.is_empty());
    assert_eq!(scan.summary(), "ortseam-mcb.exe is not beside ortseam");
