//! Integration tests that run the built `ortseam` binary.

use std::path::{Path, PathBuf};
use std::process::Command;

fn workspace() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("ortseam-cli-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    dir
}

fn run(args: &[&str]) -> (String, String, bool) {
    let output = Command::new(env!("CARGO_BIN_EXE_ortseam"))
        .args(args)
        .output()
        .expect("running ortseam");
    (
...
fn sample(path: &Path) {
    ...
    let mut spectrum =
        ortseam_core::Spectrum::from_counts(counts.into_iter().map(|c| c.round() as u64).collect());
...
fn help_and_version_work() {
    ...
    assert!(ok);
    assert!(stdout.contains("ortseam"), "{stdout}");
}
...
fn convert_moves_data_between_formats() {
    ...
    let original = ortseam_formats::load_spectrum(&chn).unwrap();
    for path in [&spe, &json, &spc, &text] {
        let converted = ortseam_formats::load_spectrum(path).unwrap();
        assert_eq!(
            converted.channels,
            original.channels,
            "{} lost data",
...
fn calibrate_fits_and_saves() {
    ...
    let calibrated = ortseam_formats::load_spectrum(&out).unwrap();
...
fn acquire_collects_from_the_simulator() {
    ...
    assert!(stdout.contains("keV"), "{stdout}");
    let spectrum = ortseam_formats::load_spectrum(&out).unwrap();
...
fn acquire_is_repeatable_for_a_given_seed() {
    ...
    for run_index in 0..2 {
        ...
        assert!(ok, "{stderr}");
        totals.push(ortseam_formats::load_spectrum(&out).unwrap().channels);
    }
...
",
    ...
    for index in 1..=2 {
        ...
        let spectrum = ortseam_formats::load_spectrum(&saved).unwrap();
