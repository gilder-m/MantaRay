...
//! program was started from: every fetch's declared count, its sum, its
//! clocks, and what the mirror did about it; every command and its answer.
...
fn journal() -> Option<&'static Mutex<std::fs::File>> {
    JOURNAL
        .get_or_init(|| {
            ...
            let mut file = std::fs::File::create("mantaray-debug.log").ok()?;
            let _ = writeln!(
                file,
