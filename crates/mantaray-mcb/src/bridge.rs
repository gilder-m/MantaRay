...
fn usage() {
    println!(
        "mantaray-mcb - the 32-bit bridge to ORTEC hardware\n\n\
         USAGE:\n  \
           mantaray-mcb probe [--umcbi-dir DIR]\n  \
           mantaray-mcb talk <detector> <command...> [--umcbi-dir DIR]\n\n\
...
fn probe(directory: Option<&str>) -> Result<(), String> {
    ...
    if std::env::var("MANTARAY_MCB_DEBUG").is_ok() {
...
/// mantaray and MAESTRO both read.
fn dump(directory: Option<&str>, arguments: &[String]) -> Result<(), String> {
    ...
    let mut spectrum = mantaray_core::Spectrum::from_counts(counts);
    ...
    // The same calibration the bridge hands to mantaray when serving live, so a
    ...
    match serve::calibration_for(mcb) {
        Some((a, b, c)) => {
            ...
            spectrum.energy_calibration =
                Some(mantaray_core::EnergyCalibration::new([a, b, c], "keV"));
        }
...
/// Serves one detector to mantaray over standard input and output.
///
...
fn serve_bridge(directory: Option<&str>, arguments: &[String]) -> Result<(), String> {
    ...
    // so a person watching sees what was reached without disturbing mantaray.
    eprintln!(
...
fn configure(directory: Option<&str>, arguments: &[String]) -> Result<(), String> {
    ...
    if path.exists() && !arguments.iter().any(|argument| argument == "--replace") {
        let backup = path.with_extension("INI.before-mantaray");
        if !backup.exists() {
...
///
/// `mantaray-mcb usbdump 0x0200 64` reads sixty-four bytes from that offset.
...
///
/// `mantaray-mcb usbscan` reads a block from every offset in turn and reports
...
///
/// `mantaray-mcb usbspectrum --out FILE` does what `dump` does, except that it
...
fn usb_spectrum(mut arguments: Vec<String>) -> Result<(), String> {
    ...
    if let Some(path) = output {
        let mut spectrum =
            mantaray_core::Spectrum::from_counts(counts.iter().map(|c| u64::from(*c)).collect());
        ...
        for (at, inside) in regions.iter().enumerate() {
            if *inside {
                spectrum.rois.mark(mantaray_core::Roi::new(at, at));
            }
        }
        mantaray_formats::save_spectrum(&spectrum, &path)
...
///
/// `mantaray-mcb usbraw 03 01 00 08 02 00 00` writes those bytes to endpoint 1
