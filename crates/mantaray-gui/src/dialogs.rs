...
pub fn menu_bar(app: &mut App, ui: &mut egui::Ui) -> Vec<Action> {
    ...
    egui::MenuBar::new().ui(ui, |ui| {
        ...
        ui.menu_button("Services", |ui| {
            ...
            if cfg!(windows) {
                ...
                if ui
                    ...
                    .on_hover_text(
                        "double-clicking .Spe, .Chn or .Spc in Explorer opens mantaray \
        ...
        ui.menu_button("Display", |ui| {
            ...
            ui.separator();
            if ui.button("Peak Labels").clicked() {
                actions.push(Action::TogglePeakLabels);
                ui.close();
            }
        ...
        ui.menu_button("Help", |ui| {
            ...
            if ui.button("About mantaray...").clicked() {
...
pub fn status_sidebar(app: &mut App, ui: &mut egui::Ui) -> Vec<Action> {
                ...
                let (rows, fingerprint) = match cache {
                    ...
                    _ => {
                        let rows: Vec<(usize, usize, usize, String)> = spectrum
                            ...
                            .enumerate()
                            .map(|(index, roi)| {
                                let info = mantaray_core::peak_info(spectrum, *roi, &settings).ok();
    ...
    ui.with_layout(egui::Layout::bottom_up(egui::Align::Center), |ui| {
        ui.add_space(6.0);
        ui.label(egui::RichText::new(crate::DISPLAY_NAME).weak());
        ui.label(
            egui::RichText::new(
                chrono::Local::now()
                ...
                2 => {
                    ...
                    if ui.checkbox(&mut mda_on, "MDA preset").changed() {
                        presets.mda = mda_on.then_some(MdaPreset {
                            target: 10.0,
                            region: mantaray_core::Roi::new(0, 100),
                            efficiency: 0.05,
                            yield_fraction: 0.85,
                        });
                    }
...
pub(crate) fn strongest_centroid(nuclide: &mantaray_core::NuclideResult) -> Option<f64> {
    nuclide
        .lines
        .iter()
        .max_by(|a, b| {
...
fn analysis_dialog(app: &mut App, ctx: &egui::Context, actions: &mut Vec<Action>) {
    dialog_window(app, ctx, Dialog::Analysis, "Nuclide analysis", |app, ui| {
        ...
        ui.horizontal(|ui| {
            ...
            {
                let _ = std::fs::write(path, mantaray_report::results_csv(report));
            }
        });
        ui.separator();
        match &app.analysis {
...
fn library_dialog(app: &mut App, ctx: &egui::Context, actions: &mut Vec<Action>) {
    dialog_window(app, ctx, Dialog::Library, "Nuclide library", |app, ui| {
        ui.horizontal(|ui| {
            ...
            {
                match mantaray_formats::library::save(&app.library, &path) {
...
fn detector_list_dialog(app: &mut App, ctx: &egui::Context, actions: &mut Vec<Action>) {
    dialog_window(
        ...
        |app, ui| {
            ...
            ui.label(
                egui::RichText::new(
                    "an MCB served over TCP - another mantaray's simulator, a gateway, \
        ...
        |app, ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button("Apply").clicked() {
                    let range = mantaray_formats::ListRange::from_seconds(
...
fn about_dialog(app: &mut App, ctx: &egui::Context) {
    dialog_window(app, ctx, Dialog::About, "About mantaray", |_app, ui| {
        ui.heading(crate::DISPLAY_NAME);
...
fn stability_trace(app: &mut App, ui: &mut egui::Ui) {
    let Some(number) = app
        ...
        .map(|mcb| mantaray_device::Mcb::identity(mcb).number)
    else {
        return;
    };
...
fn recover_dialog(app: &mut App, ctx: &egui::Context) {
    ...
    egui::Window::new("Unfinished work")
        ...
        .show(ctx, |ui| {
            ui.label("mantaray did not exit cleanly last time.");
            ui.add_space(4.0);
