...
fn region_peak(spectrum: &Spectrum, region: mantaray_core::roi::Roi) -> Option<(usize, u64)> {
...
fn draw_isotope_lines(
    ...
    let mut lines: Vec<&mantaray_core::LibraryPeak> = nuclide
        .peaks
        .iter()
    ...
    // On a chip rather than bare text. This sits in the busiest corner of the
    // plot - peak markers put the library's name for a peak at the top of every
    // tall one - and two overlapping names in the same colour read as neither.
    let anchor = Pos2::new(plot.left() + 6.0, plot.top() + 4.0);
    let galley = painter.layout_no_wrap(label, FontId::proportional(11.0), color);
    let chip = Rect::from_min_size(anchor, galley.size()).expand2(Vec2::new(4.0, 2.0));
    painter.rect_filled(
        chip,
        egui::CornerRadius::same(3),
        colors.background.to_color().gamma_multiply(0.82),
    );
