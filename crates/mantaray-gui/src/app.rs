...
use mantaray_device::SimulatedMcb;
use mantaray_device::{
...
use mantaray_report::{ReportOptions, nuclide_report, print_spectrum, roi_report};
...
fn open_with_default_app(path: &std::path::Path) -> Result<(), String> {
    ...
    if std::env::var_os("MANTARAY_NO_OPEN").is_some() {
        return Ok(());
    }
...
pub enum Action {
    ...
    /// Name found peaks on the plot, or leave only the carets.
    TogglePeakLabels,
    ToggleEnergyAxis,
...
impl IsotopeCheck {
    ...
    pub fn resolve(&mut self, library: &NuclideLibrary) {
        ...
        match library.find_typed(typed) {
            ...
            None => {
                let read_as = mantaray_core::parse_nuclide_name(typed);
...
pub struct App {
    ...
    pub job_variables: mantaray_jobs::JobVariables,
...
impl App {
    ...
    pub fn look_for_unfinished_work(&mut self) {
        ...
        if let Some(session) = &self.recovered {
            ...
            self.status = format!(
                "mantaray did not exit cleanly last time; {count} window(s) can be reopened"
            );
    ...
    pub fn headless() -> Self {
        ...
        let opening_advice = if detectors.is_empty() {
            "mantaray ready. File/Recall opens a spectrum; \
             Services/Detector List connects an instrument."
        } else {
            "mantaray ready. Acquire/Start begins a count; File/Recall opens a spectrum."
        };
        let mut app = Self {
            ...
            job: None,
            job_wait: None,
            job_variables: mantaray_jobs::JobVariables::new(),
    ...
    /// What the active window has on screen: zoom, marker, scale, toggles.
    pub fn active_display(&self) -> Option<&crate::viewmodel::DisplayState> {
        self.active
    ...
    fn tick(&mut self) {
        ...
        if !self.looked_for_instruments && self.shown {
            ...
            if self.detectors.is_empty() && std::env::var("MANTARAY_NO_OPEN").is_err() {
    ...
    pub fn advance_by(&mut self, elapsed: f64) {
        ...
        for (number, kind) in stopped {
            ...
            if self
                .queue
                .current()
                .is_some_and(|job| job.state == mantaray_device::JobState::Counting)
            {
    ...
    fn apply_one(&mut self, action: Action) {
        match action {
            ...
            Action::CopyReport => {
                if let Some(spectrum) = self.active_spectrum() {
                    let options = ReportOptions {
                        format: mantaray_report::ReportFormat::Column,
            ...
            Action::CopyCsv => {
                if let Some(spectrum) = self.active_spectrum() {
                    self.pending_clipboard = Some(mantaray_report::spectrum_csv(spectrum));
            ...
            Action::TogglePeakLabels => {
                self.with_display(|display| display.peak_labels = !display.peak_labels)
            }
    ...
    fn toggle_list_mode(&mut self) {
        ...
        let mode = if detector.mode() == mantaray_device::AcquisitionMode::List {
    ...
    fn toggle_zdt(&mut self) {
        ...
        let mode = match detector.mode() {
            mantaray_device::AcquisitionMode::Zdt => mantaray_device::AcquisitionMode::Pha,
            _ => mantaray_device::AcquisitionMode::Zdt,
        };
        match detector.set_mode(mode) {
            Ok(()) => self.status = format!("collection mode: {}", mode.label()),
            Err(error) => self.status = error.to_string(),
        ...
        let region = selection
            ...
            .or_else(|| {
                ...
                let (centre, fwhm) = match spectrum.fwhm_at(marker as f64) {
                    ...
                    None => mantaray_core::peak_search(spectrum, &settings)
                        .into_iter()
    ...
    fn record_qa(&mut self) {
        ...
        for (metric, value) in [
            (mantaray_core::QaMetric::Centroid, centroid),
            (mantaray_core::QaMetric::Fwhm, fwhm),
            (
                mantaray_core::QaMetric::NetRate,
                if live_time > 0.0 {
                    net / live_time
                } else {
                    0.0
            ...
            let chart = match self.qa.iter_mut().find(|chart| chart.metric == metric) {
                Some(chart) => chart,
                None => {
                    self.qa.push(QaChart::new(
                        metric,
                        mantaray_core::ControlLimits::default(),
                    ));
                    self.qa.last_mut().expect("just pushed")
                }
            };
            chart.push(mantaray_core::QaRecord::new(timestamp, value));
        }
    ...
    fn register_file_types(&mut self) {
        #[cfg(windows)]
        {
            ...
            self.status = "double-clicking .Spe, .Chn or .Spc now opens mantaray".into();
        }
        #[cfg(not(windows))]
        {
        ...
        match load_spectrum(&path) {
            Ok(spectrum) => {
                ...
                let list = if SpectrumFormat::from_path(&path) == Some(SpectrumFormat::List) {
                    std::fs::read(&path)
                        .ok()
                        .and_then(|bytes| mantaray_formats::list_mode::read(&bytes).ok())
                } else {
                    None
                };
                self.status = format!("{}: {}", title, mantaray_report::summary(&spectrum));
    ...
    fn export(&mut self) {
        ...
        {
            mantaray_report::spectrum_csv(&spectrum)
        } else {
            mantaray_formats::ascii::write(&spectrum, &Default::default())
        };
    ...
    pub fn print_document(&self) -> Option<String> {
        ...
        let report = mantaray_report::roi_report(spectrum, &options);
    ...
    fn print_to_browser(&mut self) {
        ...
        let path = std::env::temp_dir().join("mantaray-print.html");
    ...
    pub fn efficiency_points_from_source(
        ...
        elapsed_days: f64,
    ) -> Result<Vec<mantaray_core::EfficiencyPoint>, String> {
        let nuclide = self
            .library
            .iter()
        ...
        let source = mantaray_core::CertifiedSource {
        ...
        for line in &nuclide.peaks {
            ...
            let Ok(info) = mantaray_core::peak_info(spectrum, *roi, &settings) else {
                continue;
            };
    ...
    pub fn auto_calibrate(&mut self, nuclide: &str) {
        ...
        match mantaray_core::auto_calibrate(&centroids, &lines, "keV") {
            Ok(result) => {
    ...
    fn load_library(&mut self) {
        ...
        match mantaray_formats::library::load(&path) {
            Ok(library) => {
    ...
    ) -> Result<(), mantaray_jobs::JobError> {
        match self.job_detector() {
            Some(detector) => {
                edit(&mut self.detectors[detector]);
                Ok(())
            }
            None => Err(mantaray_jobs::JobError::host("no detector is configured")),
        }
    }
    ...
    /// file wants to see. `MANTARAY_DEMO=1` additionally marks the peaks and opens
    ...
    pub fn settle_layout(&mut self) {
        ...
        let mut demo = std::env::var("MANTARAY_DEMO").unwrap_or_default();
        ...
        match demo.as_str() {
            ...
            "dialogs" => {
                ...
                self.queue
                    .push(mantaray_device::SampleJob::new("S001", 300.0));
            ...
            "charts" => {
                ...
                let mut chart = QaChart::new(
                    mantaray_core::QaMetric::Centroid,
                    mantaray_core::ControlLimits::default(),
                );
                for (day, value) in [
                ...
                .enumerate()
                {
                    chart.push(mantaray_core::QaRecord::new(
                ...
                self.dialogs.open(Dialog::Qa);
                let points = vec![
                    mantaray_core::EfficiencyPoint::new(59.5, 0.012),
                    mantaray_core::EfficiencyPoint::new(122.1, 0.058),
                    mantaray_core::EfficiencyPoint::new(356.0, 0.035),
                    mantaray_core::EfficiencyPoint::new(661.7, 0.021),
                    mantaray_core::EfficiencyPoint::new(1173.2, 0.013),
                    mantaray_core::EfficiencyPoint::new(1332.5, 0.0115),
                ];
                self.dialogs.efficiency_points = points
                    .iter()
                    .map(|point| (point.energy.to_string(), point.efficiency.to_string()))
                    .collect();
                if let Ok(curve) = mantaray_core::EfficiencyCalibration::fit_log_log(&points, 3) {
            ...
            "hardware" => {
                // A real instrument through the bridge. MANTARAY_UMCBI_DIR names
                ...
                let umcbi = std::env::var("MANTARAY_UMCBI_DIR").unwrap_or_default();
                let number = std::env::var("MANTARAY_MCB_DETECTOR")
                    .ok()
                ...
                self.add_detector(mantaray_device::DetectorEntry {
                    number: 1,
            ...
            "nolabels" => {
                ...
                // exists for, and only a picture shows whether the carets
                // still read as markers once the names are gone.
                self.apply_one(Action::PeakSearch);
                self.apply_one(Action::TogglePeakLabels);
                return;
            }
            "empty" => {
            ...
            "isotope" => {
                ...
                // the sidebar's Isotope box is for. MANTARAY_DEMO_ISOTOPE picks
    ...
    pub fn open_arguments(&mut self, files: &[PathBuf]) {
        for path in files {
            ...
            match extension.as_str() {
                ...
                "lib" | "csv" => match mantaray_formats::library::load(path) {
                    Ok(library) => {
    ...
    fn run_bridge(&mut self, arguments: &[&str]) -> Result<String, String> {
        let executable = mantaray_device::BridgeTransport::find_executable().ok_or_else(|| {
            format!(
                ...
                mantaray_device::BRIDGE_EXECUTABLE
            )
        })?;
        let mut command = std::process::Command::new(&executable);
        command.args(arguments);
        mantaray_device::no_console(&mut command);
    ...
    fn next_detector_number(&self) -> u16 {
        (1..=mantaray_device::MAX_DETECTOR_NUMBER)
            .find(|candidate| {
                !self
                    .detector_list
                    .entries()
    ...
    fn edit_calibration_point(&mut self, index: usize, channel: f64, value: f64) {
        ...
        if window
            .calibration
            .set(index, mantaray_core::CalibrationPoint::new(channel, value))
        {
    ...
    pub fn add_detector(&mut self, entry: DetectorEntry) {
        let mcb: AnyMcb = match entry.kind {
            mantaray_device::DetectorKind::Network => {
                ...
                let connected = mantaray_device::TcpTransport::connect(
                    &address,
                    std::time::Duration::from_secs(3),
                )
                .and_then(|transport| {
                    mantaray_device::RemoteMcb::connect(
                        Box::new(transport),
                        entry.number,
                        &entry.name,
                    )
            ...
            mantaray_device::DetectorKind::Local => {
                ...
                let Some(executable) = mantaray_device::BridgeTransport::find_executable() else {
                    self.status = format!(
                        "{} is not beside mantaray - the bridge to ORTEC hardware is missing",
                        mantaray_device::BRIDGE_EXECUTABLE
                    );
                    return;
                };
            ...
            mantaray_device::DetectorKind::FileReplay => {
                self.status =
                    "file replay is not built into this version - use File/Recall instead".into();
                return;
            }
            #[cfg(feature = "simulator")]
            mantaray_device::DetectorKind::Simulated => {
                SimulatedMcb::builder(entry.number, &entry.name)
                    .channels(entry.channels)
                    .build()
                    .into()
...
impl eframe::App for App {
    ...
    fn save(&mut self, storage: &mut dyn eframe::Storage) {
        if let Ok(text) = serde_json::to_string(&self.persisted()) {
            storage.set_string(crate::APPLICATION_ID, text);
        }
    }
