...
//! `crates/ortseam-formats/tests/fixtures/` and these tests will exercise the
...
use ortseam_formats::{SpectrumFormat, load_spectrum};
...
fn fixture_spectra_survive_a_native_round_trip() {
    ...
    for path in files {
        ...
        let text = ortseam_formats::native::write(&original).unwrap();
        let back = ortseam_formats::native::read(&text).unwrap();
...
fn fixture_chn_files_survive_a_chn_round_trip() {
    for path in fixtures() {
        ...
        let original = load_spectrum(&path).unwrap();
        let bytes = ortseam_formats::chn::write(&original).unwrap();
        let back = ortseam_formats::chn::read(&bytes).unwrap();
