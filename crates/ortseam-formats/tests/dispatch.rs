...
use ortseam_core::{EnergyCalibration, Roi, Spectrum};
use ortseam_formats::{
    ListModeFile, ListRange, SpectrumFormat, list_mode, load_spectrum, save_spectrum,
};

fn temp_dir() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("ortseam-tests-{}", std::process::id()));
...
fn saving_a_region_file_next_to_a_spectrum() {
    ...
    ortseam_formats::save_rois(&s.rois, &path).unwrap();
    let back = ortseam_formats::load_rois(&path).unwrap();
