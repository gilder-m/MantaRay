...
//! with `MANTARAY_ORTEC_EXAMPLES`; the default is where the reference copy lives
//! on the machine this was written on.
//!
//! ```text
//! MANTARAY_ORTEC_EXAMPLES="D:\V9 Examples" cargo test -p mantaray-formats --test ortec_examples
//! ```
...
///
/// Only `MANTARAY_ORTEC_EXAMPLES` says; there is no default, because a default
/// would be one machine's layout written into everyone's checkout.
fn examples() -> Option<PathBuf> {
    let directory = PathBuf::from(std::env::var("MANTARAY_ORTEC_EXAMPLES").ok()?);
    ...
    if examples().is_none() {
        eprintln!(
            "skipping: ORTEC's example files are not installed. \
             Set MANTARAY_ORTEC_EXAMPLES to the V9 Examples directory to run this."
        );
        return true;
    }
    false
...
fn every_example_spectrum_loads() {
    ...
    for path in &files {
        if let Err(error) = mantaray_formats::load_spectrum(path) {
...
fn every_example_spectrum_holds_counts_and_a_clock() {
    ...
    for path in spectra(&["spc", "chn", "n42"]) {
        let spectrum = mantaray_formats::load_spectrum(&path).expect("it loaded a moment ago");
...
fn a_channel_count_is_never_a_strange_number() {
    ...
    for path in spectra(&["spc", "chn", "n42"]) {
        let spectrum = mantaray_formats::load_spectrum(&path).expect("it loaded");
...
fn a_calibrated_example_puts_its_peaks_at_real_energies() {
    ...
    for (name, energy, line) in checks {
        ...
        let mut spectrum = mantaray_formats::load_spectrum(&path).expect("it loads");
        let calibration = spectrum
            .energy_calibration
            .clone()
            .unwrap_or_else(|| panic!("{name} should carry a calibration"));
        let settings = mantaray_core::CalculationSettings::default();
        mantaray_core::mark_peaks(&mut spectrum, &settings);
        let near = spectrum
            .rois
            .iter()
...
fn an_spc_and_its_chn_agree_where_both_exist() {
    ...
    for path in spectra(&["spc"]) {
        ...
        let (Ok(spc), Ok(chn)) = (
            mantaray_formats::load_spectrum(&path),
            mantaray_formats::load_spectrum(&twin),
        ) else {
            continue;
        };
        let name = path.file_name().unwrap_or_default().to_string_lossy();
...
fn every_example_spectrum_survives_a_native_round_trip() {
    ...
    // Whatever mantaray reads, it must be able to write and read back without
    // losing anything - that is what makes the native format worth having.
    let directory = std::env::temp_dir().join("mantaray-ortec-examples");
    std::fs::create_dir_all(&directory).expect("a scratch directory");
    for path in spectra(&["spc", "chn", "n42"]) {
        let original = mantaray_formats::load_spectrum(&path).expect("it loads");
        let name = path.file_name().unwrap_or_default().to_string_lossy();
        let out = directory.join(format!("{name}.json"));
        mantaray_formats::save_spectrum(&original, &out).expect("it writes");
        let again = mantaray_formats::load_spectrum(&out).expect("it reads back");
        assert_eq!(
...
fn every_shipped_binary_library_reads_whole_and_sane() {
        ...
        .flatten()
    {
        ...
        checked += 1;
        let name = path.file_name().unwrap_or_default().to_string_lossy();
        let library = mantaray_formats::library::load(&path)
...
fn the_shipped_suspect_library_reads_back_its_known_contents() {
    ...
    let library = mantaray_formats::library::load(&path).expect("Suspect.Lib reads");
    ...
    assert!(
        cs.peaks.iter().any(
            |p| (p.energy - 661.66).abs() < 0.1 && p.photon == mantaray_core::PhotonKind::Gamma
        ),
    ...
    assert!(
        cs.peaks
            .iter()
            .any(|p| (p.energy - 32.194).abs() < 0.1 && p.photon == mantaray_core::PhotonKind::XRay),
    ...
    assert!(
        na.peaks
            ...
            .any(|p| (p.energy - 511.0).abs() < 0.1
                && p.photon == mantaray_core::PhotonKind::Positron),
