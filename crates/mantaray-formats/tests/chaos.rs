...
use mantaray_formats::{SpectrumFormat, read_as, sniff};
...
fn every_reader_survives_truncations_of_a_valid_file() {
    ...
    let whole: Vec<(SpectrumFormat, Vec<u8>)> = vec![
        (
            SpectrumFormat::Chn,
            mantaray_formats::chn::write(&spectrum).expect("chn"),
        ),
        (
            SpectrumFormat::Spe,
            mantaray_formats::spe::write(&spectrum)
                .expect("spe")
                .into_bytes(),
        ),
        (
            SpectrumFormat::Native,
            mantaray_formats::native::write(&spectrum)
                .expect("native")
                .into_bytes(),
        ),
    ];
...
fn a_declared_length_cannot_force_an_allocation() {
    ...
    let spe = "$SPEC_ID:\nchaos\n$DATA:\n0 900000000\n0\n1\n2\n";
    assert!(
        mantaray_formats::spe::read(spe).is_err(),
        ".Spe must refuse the range"
    );
    // And a first channel past the sixteen-bit offset a spectrum can carry.
    let offset = "$DATA:\n70000 70003\n0\n1\n2\n3\n";
    assert!(
        mantaray_formats::spe::read(offset).is_err(),
        ".Spe must refuse a first channel past u16"
    );

    // List mode: a valid file with the channel-count field rewritten to u32::MAX.
    let mut list = mantaray_formats::ListModeFile::new(1024);
    list.push(0.5, 100);
    let mut bytes = mantaray_formats::list_mode::write(&list).expect("write");
    bytes[12..16].copy_from_slice(&u32::MAX.to_le_bytes());
    assert!(
        mantaray_formats::list_mode::read(&bytes).is_err(),
    ...
    assert!(
        mantaray_formats::n42::from_str(&runs).is_err(),
...
fn corrupt_bodies_behind_valid_headers_are_survived() {
    let mut spectrum = mantaray_core::Spectrum::new(64);
    spectrum.channels[10] = 1000;
    let clean = mantaray_formats::chn::write(&spectrum).expect("chn");
    for seed in 1..=30u64 {
