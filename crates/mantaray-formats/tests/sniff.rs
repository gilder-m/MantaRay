...
use mantaray_formats::{SpectrumFormat, load_spectrum, sniff};
...
fn a_chn_spectrum_is_recognised_by_its_leading_marker() {
    let spectrum = mantaray_core::Spectrum::new(256);
    let bytes = mantaray_formats::chn::write(&spectrum).expect("write .Chn");
    assert_eq!(sniff(&bytes), Some(SpectrumFormat::Chn));
}

#[test]
fn native_json_is_recognised_by_its_contents() {
    let spectrum = mantaray_core::Spectrum::new(16);
    let text = mantaray_formats::native::write(&spectrum).expect("write native");
...
fn a_list_mode_file_is_recognised_by_its_magic() {
    let file = mantaray_formats::ListModeFile::new(64);
    let bytes = mantaray_formats::list_mode::write(&file).expect("write list mode");
