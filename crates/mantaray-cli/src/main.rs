//! `mantaray` - the headless workbench: convert, analyse, report, automate and
...
#[cfg(feature = "simulator")]
use mantaray_device::{Mcb, Presets, SimulatedMcb, advance};
use mantaray_formats::{
    AsciiOptions, SpectrumFormat, ascii, library, load_spectrum, save_spectrum,
};
use mantaray_report::{
...
#[derive(Parser, Debug)]
#[command(name = "mantaray", version, about, long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Library {
        ...
        #[arg(long, default_value_t = mantaray_formats::nndc::DEFAULT_MIN_INTENSITY)]
        min_intensity: f64,
    ...
    ///
    /// Another mantaray (or anything speaking the SET_/SHOW_ dialect) can then
...
fn job_instrument(address: Option<&str>) -> Result<Option<mantaray_device::AnyMcb>> {
    if let Some(address) = address {
        let transport =
            mantaray_device::TcpTransport::connect(address, std::time::Duration::from_secs(5))
                .with_context(|| format!("connecting to {address}"))?;
        let remote = mantaray_device::RemoteMcb::connect(Box::new(transport), 1, "JOB")
...
fn serve(address: &str, channels: usize) -> Result<()> {
    ...
    mantaray_device::server::serve(listener, instrument).context("serving")?;
    Ok(())
}
...
fn build_library(export: &Path, min_intensity: f64, output: &Path) -> Result<()> {
    ...
    let built = mantaray_formats::nndc::build(&text, min_intensity)
...
fn analyse_command(
    ...
    if library.is_empty() {
        bail!(
            "no nuclide library: give --library <file.Lib>. mantaray ships none, \
