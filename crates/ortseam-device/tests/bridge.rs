...
//!
//! The helper is a shell script rather than the real `ortseam-mcb`, because
...
use ortseam_device::BridgeTransport;
use ortseam_device::transport::Transport;
...
fn helper(name: &str) -> PathBuf {
    ...
    DIRECTORY
        .get_or_init(|| {
            let directory =
                std::env::temp_dir().join(format!("ortseam-bridge-{}", std::process::id()));
