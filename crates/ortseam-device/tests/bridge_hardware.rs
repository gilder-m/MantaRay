//! The whole chain against a real instrument: ORTSEAM's dialect, through the
...
//!
//! Point it at the bridge with `ORTSEAM_MCB`; it is skipped entirely without
//! that, because guessing at a path would silently test nothing.
//!
//! ```text
//! ORTSEAM_MCB=target/i686-pc-windows-msvc/release/ortseam-mcb.exe \
//!   cargo test -p ortseam-device --test bridge_hardware -- --nocapture
//! ```

use std::path::PathBuf;
use std::sync::{Mutex, MutexGuard};

use ortseam_device::{BridgeTransport, Transport};
...
fn bridge() -> Option<PathBuf> {
    let path = PathBuf::from(std::env::var("ORTSEAM_MCB").ok()?);
...
fn the_whole_chain_reaches_a_real_instrument() {
    ...
    let Some(path) = bridge() else {
        assert!(skip("ORTSEAM_MCB does not name a bridge executable"));
        return;
    };
    ...
    // What the instrument is. A configuration line is the first thing ORTSEAM
    ...
    // The clocks, and the arithmetic ORTSEAM does on them.
